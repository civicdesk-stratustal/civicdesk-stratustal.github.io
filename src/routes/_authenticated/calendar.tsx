...
export const Route = createFileRoute("/_authenticated/calendar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "See every upcoming deadline on a month view and in a dated list." },
...
function CalendarPage() {
  ...
  return (
    <div className="safe-top px-5">
      <h1 className="mb-4 text-xl font-bold text-white">Calendar</h1>
      ...
      <GlassCard className="mb-5">
        <div className="mb-4 flex items-center justify-between">
          <button
            ...
            className="press flex h-11 w-11 items-center justify-center rounded-full bg-white/[0.06] text-white/80"
          >
        ...
        <div className="mb-2 grid grid-cols-7 text-center text-[10px] text-white/40">
          {DAY_LABELS.map((d, i) => (
            <span key={i}>{d}</span>
          ))}
        </div>
            ...
            return (
              <span
                ...
                className={`mx-auto flex h-9 w-9 flex-col items-center justify-center rounded-full text-xs ${
                  isToday ? "bg-primary font-semibold text-white" : "text-white/80"
                }`}
              >
                {day}
                {marked.has(day) ? (
      ...
      <h2 className="mb-3 text-sm font-semibold text-white/80">Upcoming dates</h2>
      <div className="space-y-3">
        {upcoming.length === 0 ? (
          <GlassCard className="text-center text-sm text-white/60">No upcoming deadlines.</GlassCard>
        ) : (
          upcoming.map((d) => (
            <GlassCard key={d.id}>
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
                ...
                <span className="shrink-0 text-xs text-white/70">{timeRemaining(d)}</span>
              </div>
            </GlassCard>
          ))
        )}
